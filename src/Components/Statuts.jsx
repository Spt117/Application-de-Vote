...
export default function Statuts({ statut }) {
   ...
   useEffect(() => {
      if (statut !== undefined) set()
      // eslint-disable-next-line
   }, [statut])

   function set() {
      const thisClass = [
         { thestatut: 0, idClass: nextStatut },
         { thestatut: 1, idClass: nextStatut },
         { thestatut: 2, idClass: nextStatut },
         { thestatut: 3, idClass: nextStatut },
         { thestatut: 4, idClass: nextStatut },
         { thestatut: 5, idClass: nextStatut },
      ]

      for (let j = 0; j < 6; j++) {
         if (statut === thisClass[j].thestatut) thisClass[j].idClass = currentStatut
         else if (statut < thisClass[j].thestatut) thisClass[j].idClass = nextStatut
         else if (statut > thisClass[j].thestatut) thisClass[j].idClass = oldStatut
      }
   ...
   if (theClass !== undefined)
      return (
         <div className="statuts">
            <h3 id="hstatuts">Statut de la session de vote</h3>
